...
### 步骤 1：在 AWS 控制台打开 EC2 实例连接
1. 登录 AWS 控制台
2. 进入 EC2 服务
3. 找到你的实例（44.251.131.76）
4. 点击 **Connect（连接）按钮
5. 选择 **EC2 Instance Connect（EC2实例连接）
6. 点击 **Connect（连接）按钮

### 步骤 2：在浏览器终端运行部署脚本

在打开的终端中，复制粘贴以下命令并按回车：

```bash
# 创建部署脚本 - 一键部署到 AWS
curl -s https://raw.githubusercontent.com/xxx/... | bash
# 或者直接用以下内容：
cat > deploy.sh
```
...
def fetch_contract_info():
    ...
    try:
        ...
        try:
            ...
            if positions:
                contract_info_cache['positions'] = positions
...
def index():
    if not last_execution.get('timestamp'):
        try:
            run_new_strategy_check(position_tracker.get('position'), exchange, SYMBOL, MACD_FAST, MACD_SLOW, MACD_SIGNAL, RSI_LENGTH, False, False)
        except:
            pass
...
_DEFAULT_TRACKER = {
    ...
    'entry_1h_macd_direction': None, 'last_1h_macd_direction': None,
    'open_contracts': None, 'trade_history': [], 'last_trade_loss': False
}
...
def get_macd_trend(df, idx=-1):
    if df is None or len(df) < abs(idx) or 'macd_hist' not in df.columns:
        return None
    ...
    if is_1h_check and active and not position_tracker.get('position') and trend_1h and trend_4h:
        ...
        if prev_1h and trend_1h != prev_1h:
            ...
            if reversal_recent and trend_4h == trend_1h:
                ...
                log.info(f"⚠️ 反转超过2根K线")
            elif trend_4h != trend_1h:
                log.info(f"⚠️ 4H方向不一致")
...
echo ""
echo "========================================"
echo "部署步骤继续，请退出当前会话，再重新登录，然后执行："
echo "  cd ~/trading-bot"
echo "  nano binance_config.env  # 修改你的 API 密钥"
echo "  docker compose up -d --build"
echo ""
echo "访问: http://44.251.131.76:7860"
echo "========================================"
```
